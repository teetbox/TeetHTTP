...
class TeetHTTPTests: XCTestCase {
    ...
    func testInitWithConfig() {
        sut = TeetHTTP(base: fakeBase, config: .standard)
        XCTAssertEqual(sut.config, .standard)
        
        sut = TeetHTTP(base: fakeBase, config: .ephemeral)
        XCTAssertEqual(sut.config, .ephemeral)
        
        sut = TeetHTTP(base: fakePath, config: .background("background"))
